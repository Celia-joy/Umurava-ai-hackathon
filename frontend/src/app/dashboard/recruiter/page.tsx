...
export default function RecruiterDashboard() {
  ...
  const submitJob = async (event: FormEvent<HTMLFormElement>) => {
    ...
    const payload = {
      ...
      preferredCertifications: splitCsv(
        String(formData.get("preferredCertifications"))
      ),
      projectKeywords: splitCsv(String(formData.get("projectKeywords"))),
      availabilityRequirement: String(
        formData.get("availabilityRequirement")
      ),
      ...
      <Card className="anim-fade-up p-6 md:p-8">
        ...
        {jobSuccess && (
          <div className="mb-4 rounded-2xl border border-green-200 bg-green-50 px-4 py-3 text-sm text-green-800">
            Job successfully published. Candidates can now apply and be analyzed.
          </div>
        )}
        ...
        <form onSubmit={submitJob} className="grid gap-4">
          ...
          <TextInput
            ...
            placeholder="Project keywords separated by commas"
          />
          <TextInput
            name="preferredCertifications"
            placeholder="Preferred certifications separated by commas"
          />
          <div className="grid gap-4 md:grid-cols-2">
            ...
            <TextInput
              ...
              placeholder="Availability requirement e.g. Immediate or within 2 weeks"
            />
          </div>
          ...
          <div className="mb-5 rounded-[26px] border border-brand-100 bg-brand-50/80 p-5">
            ...
            <div className="mt-3 flex flex-wrap gap-2">
              {selectedJob.requiredSkills.map((skill, index) => (
                <Pill key={`selected-job-skill-${index}-${skill}`}>
                  {skill}
                </Pill>
              ))}
            </div>
